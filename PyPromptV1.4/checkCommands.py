from programs import compile as pypr
...
#                print("INITIATING ERROR: Enter install, update or import")
        ...
        elif string[0] == "emotion":
            inp = str(input("--> "))
            try:
                import text2emotion as te
            except Exception:
                os.system("pip install text2emotion")
            finally:
                import text2emotion as te
            print(te.get_emotion(inp))
        elif string[0] == "date":
            ...
            import store
            store.main_loop()
        elif string[0] == "mcserver":
            from programs import minecraft as mc
            while True:
                uin = str(input("-> "))
                uin2 = uin.split()
                if uin[0] == ".":
                    if uin == ".quit":
                        ...
                        mc.build_server(ip=uin2[1], port=uin2[2], rconpwd=uin2[3])
                    elif uin2[0] == ".run-code":
                        pypr.run_code(uin2[1])
                elif uin[0] == "/":
                    if uin2[0] == "/say":
                        mc.send(uin2[1])
                    elif uin2[0] == "/tell":
                        mc.send(uin2[2], uin2[1])
        elif string[0] == "7857652649":
            f = open(".\\data\\main\\.mined-chunks", "w")
            f.write(str(input("Input number of Chunks... ")))
            f.close()
    ...
    store       Enters the PyPrompt Store
    ...
    emotion     Initiates the Emotion Analyzer
    write       Writes a file
